...
def test_main(mock_post_request, mock_get_issues_json, mock_get_header, mock_get_issues_url, mock_get_controls):
    mock_get_controls.return_value = [
        ["Family1", "Control1", "E1", "Control1 name", "Class1", "Definition1", "Supplemental guidance1",
            "References1", "IT Security Function", "", "", "", "", "", "Medium", "", "", ""],
        ["Family2", "Control2", "E2", "Control2 name", "Class2", "Definition2", "Supplemental guidance2",
            "References2", "", "IT Operation Group", "", "", "", "", "Low", "", "", ""]
    ]
...
def test_get_issues_json(mock_get_labels, mock_get_body, mock_get_title):
    ...
    assert issues_json == {"title": "title",
                           "body": "body", "labels": ["label1", "label2"]}
...
def test_get_body(mock_get_support_teams, mock_get_suggested_assignment):
    ...
    assert body == "#Control Definition\nThe org requires an infosec to be a member of the comitee\n\n#Class\nOperational\n\n#Supplemental Guidance\nSupplemental Guidance text\n\n#References\nReference text\n\n#General Guide\ntext\n\n#Suggested Assignment\nIT Security\n\n#Support Teams\nIT Operation Group\n\n"
...
def test_get_labels(mock_get_suggested_assignment):
    ...
    assert labels == ['Control: CM-3','Priority: P1', 'Class: Operational', 'Suggested Assignment: IT Security']
...
def test_get_controls_different_header(mock_builtins_open, mock_script_csv_reader):
    mock_script_csv_reader.return_value = iter([["a",
                                            "Control ID",
                                            "Enhancement",
                                            "Name",
                                            "Class",
...
def test_get_controls_no_controls(mock_builtins_open, mock_script_csv_reader):
    mock_script_csv_reader.return_value = iter([["Family",
                                            "Control ID",
                                            "Enhancement",
                                            "Name",
                                            "Class",
